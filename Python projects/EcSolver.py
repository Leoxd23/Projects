...
def newton_raphson(f_expr, x0, tol=1e-8, max_iter=100, usar_correccion_multiplicidad=True):
    ...
    for i in range(1, max_iter+1):
        ...
        if abs(fxi) < tol:  # ya estamos en la raíz
            xiant = xi
            error = float (abs((xi - xiant)/xi))
            print(f"Iteración {i}: x = {xi}, f(x) = {fxi}, Error aprox = {error}%")
            return xi
        ...
        if abs(fpxi) < eps_der:  
            ...
            if abs(fxi) < tol and usar_correccion_multiplicidad:
                ...
                raise ValueError(f"Derivada nula en x={xi}, cambia el valor inicial x0.  Puede que no tenga raíces")
        else:
            # Newton estándar
            delta = fxi/fpxi
        ...
        print(f"Iteración {i}: x = {xi_next}, f(x) = {float(f(xi_next))}, Error aprox = {error}")
...
def main():
    ...
    print("2) Ecuación lineal o no lineal")
